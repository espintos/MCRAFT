function MCRAFT_IRT_debug(N,tfinal,TP,PTP1,PTP2,RTP,D,R)
    ...
    """Debugging variables"""
    t_print_value = 0.1
    t_print = t_print_value
    print_counter = 1
    TimevsIters = zeros(1)
    ReacsvsIters = zeros(1)

    #Species vs Time
    species = ["tiempo","M","I","CTA","R","TP*","R*","TP","PTP","D","nothing"]
    Concentrations = zeros(iceil(t_f/t_print)+1,length(species))
    Concentrations[print_counter,:] = [t_r,M0,I0,CTA0,n_rad,n_istar,n_rstar,n_raft1,n_raft2,n_pol,n_nothing] #initial values

    #Reaction counters.
    Reacciones = zeros(Int,16) #This vector saves de number of reactions
    ReaccionesvsTiempo = zeros(iceil(t_f/t_print)+1,length(Reacciones)+1) #This vector saves the variables as a function of the reaction time
    ReaccionesvsTiempo[print_counter,:] = [t_r,Reacciones]
    #This is to print every t_print hrs of reaction time - mainly for debugging purposes.


    # Average Properties vs Time
    ...
    while t_r < t_f
     ...
     """
        ...
        if t_r > t_print
            ...
            Concentrations[print_counter,1] = t_r #overwrite it because previous was divided by Avogadrof
            ReaccionesvsTiempo[print_counter,:] = [t_r,Reacciones]
        end

    #push!(TimevsIters,t_r)
    #push!(ReacsvsIters,t_r)
  end #while


  conv = 100.0*(nmon_inic - n_mon)/(nmon_inic)
  return D,PTP1,PTP2,TP,RTP,R,conv,Concentrations,ReaccionesvsTiempo,TimevsIters
